#!/usr/bin/env python3
"""Final test of the manual MCP server."""
...
def test_manual_server():
    """Test the manual MCP server with Claude Desktop's exact setup."""
    print("🧪 Testing Manual MCP Server...")
    
    try:
        # Test with the exact same command and environment that Claude Desktop will use
        proc = subprocess.Popen(
            ['python3', '/Users/Allen/YTTL-TUI/yttl_mcp/manual_mcp_server.py'],
        ...
        if "ERROR" in stderr or "Failed to validate" in stderr:
...
def check_config():
    ...
    try:
        ...
        expected_args = ["/Users/Allen/YTTL-TUI/yttl_mcp/manual_mcp_server.py"]
        
        if yttl_config.get("args") == expected_args:
            print("✅ Claude Desktop configured to use manual_mcp_server.py")
            return True
        else:
...
def main():
    """Run all tests."""
    print("🎯 YTTL Manual MCP Server Final Test")
    print("=" * 50)
    
    tests = [
        ("Claude Desktop Configuration", check_config),
        ("Manual MCP Server", test_manual_server),
    ]
    ...
    if passed == total:
        print("🎉 All tests passed! Manual MCP Server is ready for Claude Desktop.")
        print("\n📋 Next steps:")
        print("1. Restart Claude Desktop")
        print("2. Try asking Claude: 'Search for videos about Peter Thiel'")
        print("3. Or: 'List available videos'")
        print("\n✨ The server should now work without validation errors!")
        return 0
    else:
