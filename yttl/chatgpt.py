...
async def get_new_session_async():
    """Async version of get_new_session"""
    uid = str(uuid.uuid4())
    headers = DEFAULT_HEADERS.copy()
    headers['oai-device-id'] = uid
    
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{BASE_URL}/backend-anon/sentinel/chat-requirements', 
                               headers=headers, data='{}') as resp:
            resp.raise_for_status()
            data = await resp.json()
            return (uid, data)

async def send_request_async(prompt):
    """Async version of send_request"""
    (dev_id, session) = await get_new_session_async()
    pow = session['proofofwork']
    token = generate_proof_token(pow['seed'], pow['difficulty'], USER_AGENT)
    body = {
        'action': 'next',
    ...
    async with aiohttp.ClientSession() as http_session:
        async with http_session.post(f'{BASE_URL}/backend-anon/conversation', 
                                   headers=headers, data=json.dumps(body)) as resp:
            resp.raise_for_status()
