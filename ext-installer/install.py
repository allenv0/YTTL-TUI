...
if sys.platform == 'darwin':
    ...
    winreg.SetValue(
        ...
        'Software\\Mozilla\\NativeMessagingHosts\\yttl',
        winreg.REG_SZ,
        f'{nhost_dir}\\yttl.json'
    )

os.makedirs(nhost_dir, exist_ok=True)
desc = json.load(open(f'{os.path.dirname(__file__)}/yttl.json'))
desc['path'] = os.path.join(install_dir, 'venv', bin_dir, f'yttl_ext{suffix}')
open(f'{nhost_dir}/yttl.json', 'w').write(json.dumps(desc))
venv_dir = os.path.join(install_dir, 'venv')
venv.create(venv_dir, clear=True, with_pip=True)
pip = os.path.join(venv_dir, bin_dir, 'pip')
os.execl(pip, pip, 'install', 'yttl')
